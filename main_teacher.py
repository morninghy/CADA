...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, 'checkpoint1.5.pth')):
        args.resume = os.path.join(args.output_dir, 'checkpoint1.5.pth')
    if args.resume:
        if args.resume.startswith('https'):
            checkpoint = torch.hub.load_state_dict_from_url(
                args.resume, map_location='cpu', check_hash=True)
        else:
            # 添加 weights_only=False 来解决 PyTorch 的安全警告
            checkpoint = torch.load(args.resume, map_location='cpu', weights_only=False)

        # --- 修正加载逻辑 ---
        # 1. 加载学生模型 (model)
        #    - 优先从 checkpoint['model'] 加载。
        #    - 如果 checkpoint 是第一阶段训练的，它只包含 'model'。
        #    - 使用 strict=False 来处理模型结构变化（例如添加了 confusion_matrix）。
        if 'model' in checkpoint:
            print("Loading student model from 'model' key...")
            model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
        else:
            # 如果没有 'model' 键，作为备选，尝试从 'ema_model' 加载
            print("Warning: 'model' key not found. Loading student model from 'ema_model' key...")
            model_without_ddp.load_state_dict(checkpoint['ema_model'], strict=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----2.加入伪标签,self-training训练
            ...
            # ----准备工作
            # FIX: 检查best_ema_model是否存在，如果不存在（resume时）也要进入初始化逻辑
            if epoch == cfg.burn_epochs or best_ema_model is None:

                # 仅在正好到达 burn_epochs 时才加载 transition 权重
                # 如果是 resume (epoch > burn_epochs)，通常不需要重置为 best_ema_teacher
                if epoch == cfg.burn_epochs:
                    ...
                    # （0）----------加载最优检测模型----------------
                    if os.path.exists(os.path.join(output_dir, 'best_ema_teacher.pth')):
                        checkpoint = torch.load(os.path.join(output_dir, 'best_ema_teacher.pth'), map_location='cpu')
                        if not args.distributed:
                            model_without_ddp.load_state_dict(checkpoint['ema_model'], strict=True)
                        ...
                        # 评估，用于debug
                        test_stats, coco_evaluator = evaluate(
                            ema_teacher.ema, criterion, postprocessors, data_loader_val, base_ds, device,
                            args.output_dir,
                            wo_class_error=wo_class_error, args=args, logger=(logger if args.save_log else None)
                        )
                    else:
                        print("Warning: best_ema_teacher.pth not found, skipping load.")
                ...
                # 如果是 Resume 且已经过了 burn_epochs，尝试加载之前保存的 best_ema_model
                if epoch > cfg.burn_epochs and os.path.exists(os.path.join(output_dir, 'best_ema_model.pth')):
                    print("Resuming best_ema_model from checkpoint...")
                    checkpoint = torch.load(os.path.join(output_dir, 'best_ema_model.pth'), map_location='cpu')
                    best_ema_model.ema.load_state_dict(checkpoint['ema_model'])

            # （2）----------self-training训练 with Domain Adaptation---------------)
            train_stats = train_one_epoch_with_self_training(
                model, ema_teacher, criterion, data_loader_train, data_loader_train_strong_aug, optimizer, device,
                epoch,
                args.clip_max_norm, wo_class_error=wo_class_error, lr_scheduler=lr_scheduler, args=args,
                logger=(logger if args.save_log else None), ema_m=ema_m)
        ...
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint1.5.pth']
        ...
        if args.output_dir and utils.is_main_process():
            ...
            with open(output_dir / "log_best.txt", 'w') as f:
                ...
                f.write('best_teacher -->  map50:%s , epoch:%s\n' % (best_ema_teacher_fitness,
                                                                     cache_best_ema_teacher_epoch))
