...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries,
        ...
        if self.training:
            ...
            # --- Contribution 3 Modification Start ---
            # 旧代码: 共享判别器
            # self.Proto_D = MLP(hidden_dim, hidden_dim, 1, 3)

            # 新代码: 为每个类别构建独立的域判别器
            self.class_discriminators = nn.ModuleList([
                MLP(hidden_dim, hidden_dim, 1, 3) for _ in range(num_classes)
            ])
            # --- Contribution 3 Modification End ---

            # For CAT idea: Confusion Matrix and class split for class-aware learning (Contribution 1)
            self.register_buffer('confusion_matrix', torch.zeros(self.num_classes, self.num_classes))
            # Cityscapes has 8 foreground classes with 1-based category ids in
            # the current COCO annotations. Therefore num_classes is max_id + 1,
            # where class 0 is an unused dummy slot.
            self.majority_classes = [1, 2]  # person, car
            self.minority_classes = [3, 4, 5, 6, 7, 8]  # train, rider, truck, motorcycle, bicycle, bus
            self.cm_momentum = 0.99  # Momentum for confusion matrix update
            self.cm_warmup_epochs = cm_warmup_epochs  # Warm-up epochs for confusion matrix
    ...
    def forward(self, samples: NestedTensor, targets: List = None, self_training_flag=False, epoch=0):
        ...
        # ----训练时, 拆分源域和目标域特征图
        if self.training:
            srcs_source, masks_source, poss_source, srcs_all, masks_all, poss_all, srcs_target, masks_target, poss_target = decompose_features(
                srcs, masks, poss)
            hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs_source, masks_source,
                                                                                 input_query_bbox, poss_source,
        ...
        if self.training:
            ...
            # ---- Update Confusion Matrix with target domain predictions (Contribution 1: CAT idea) ----#
            if epoch >= self.cm_warmup_epochs:
                with torch.no_grad():
                    target_pred_logits = outputs_class_target.softmax(-1)  # [bs/2, num_queries, num_classes]
                    # Use argmax as pseudo-label for each query to determine the row to update
                    pseudo_labels = target_pred_logits.argmax(-1)  # [bs/2, num_queries]

                    # Soft update of the confusion matrix
                    batch_cm = torch.zeros_like(self.confusion_matrix)
                    for i in range(pseudo_labels.shape[0]):  # batch_size
                        for j in range(pseudo_labels.shape[1]):  # num_queries
                            batch_cm[pseudo_labels[i, j]] += target_pred_logits[i, j]
                    self.confusion_matrix = self.cm_momentum * self.confusion_matrix + (1 - self.cm_momentum) * batch_cm

            # --- Contribution 3 Modification Start: Independent Class-Specific Discriminators ---
            # 旧代码：直接拼接并通过一个共享判别器
            ...
            # 新代码：分别反转梯度，遍历每个类别并通过对应的独立判别器
            proto_src_grl = grad_reverse(class_prototypes_source)  # [num_classes, hidden_dim]
            proto_tgt_grl = grad_reverse(class_prototypes_target)  # [num_classes, hidden_dim]

            da_logits_list = []
            # Part A: Source Domain
            for i in range(self.num_classes):
                # Use the i-th discriminator for the i-th class prototype
                # Input shape: [1, hidden_dim], Output shape: [1, 1]
                out_d = self.class_discriminators[i](proto_src_grl[i].unsqueeze(0))
                da_logits_list.append(out_d)

            # Part B: Target Domain
            for i in range(self.num_classes):
                out_d = self.class_discriminators[i](proto_tgt_grl[i].unsqueeze(0))
                da_logits_list.append(out_d)

            # Concatenate all logits to match the shape expected by the loss function: [2*num_classes, 1]
            class_prototypes_da = torch.cat(da_logits_list, dim=0)
            # --- Contribution 3 Modification End ---
...
class SetCriterion(nn.Module):
    ...
    def get_confused_pairs(self, confusion_matrix, majority_classes, minority_classes, top_k=1):
        """
        Extracts minority -> majority confused pairs from the confusion matrix.
        Returns a dictionary {minority_class_idx: [confused_majority_class_idx_1, ...]}
        """
        confused_pairs = {}
        # Normalize confusion matrix row-wise to get probabilities
        cm_prob = confusion_matrix / (confusion_matrix.sum(dim=1, keepdim=True) + 1e-9)

        for minority_cls in minority_classes:
            # Find the top-k majority classes this minority class is confused with
            # We only care about how minority classes are misclassified as majority classes
            confusions = cm_prob[minority_cls, majority_classes]
            if confusions.sum() > 0:
                # Ensure top_k is not larger than the number of majority classes
                k = min(top_k, len(majority_classes))
                if k > 0:
                    top_k_vals, top_k_indices = torch.topk(confusions, k=k)
                    # Map local indices within majority_classes back to global class indices
                    confused_maj_classes = [majority_classes[i] for i in top_k_indices]
                    confused_pairs[minority_cls] = confused_maj_classes
        return confused_pairs

    def loss_contrast_da_cat(self, outputs, temperature=0.2, cat_weight=1.0):
        """
        Contrastive loss with Confused-Aware-Training (CAT) inspired penalty.
        The penalty is weighted by the confusion probability.
        """
        query_source = outputs['output_source']
        query_target = outputs['outputs_target']
        query_mask_source = outputs['query_mask_source']
        query_mask_target = outputs['query_mask_target']
        global_proto = outputs['global_proto']

        # CAT-specific items
        confusion_matrix = outputs['confusion_matrix']
        majority_classes = outputs['majority_classes']
        minority_classes = outputs['minority_classes']
        ...
        # --- 1. Standard contrastive loss (InfoNCE) for source and target prototypes ---
        logits_s = query_source @ global_proto.t() / temperature
        logits_t = query_target @ global_proto.t() / temperature
        labels = torch.arange(self.num_classes, device=query_source.device)

        loss_contrast_s = F.cross_entropy(logits_s, labels, reduction='none')
        loss_contrast_s = (loss_contrast_s * query_mask_source).sum() / (query_mask_source.sum() + 1e-8)
        loss_contrast_t = F.cross_entropy(logits_t, labels, reduction='none')
        loss_contrast_t = (loss_contrast_t * query_mask_target).sum() / (query_mask_target.sum() + 1e-8)
        loss_contrast = loss_contrast_s + loss_contrast_t

        # --- 2. CAT-inspired penalty for confused pairs ---
        # Normalize confusion matrix to get probabilities
        cm_prob = confusion_matrix / (confusion_matrix.sum(dim=1, keepdim=True) + 1e-9)
        confused_pairs = self.get_confused_pairs(cm_prob, majority_classes, minority_classes)

        loss_cat = query_source.new_tensor(0.0)
        num_pairs = 0
        for minority_cls, confused_maj_list in confused_pairs.items():
            for majority_cls in confused_maj_list:
                # The penalty is the similarity of the confused pair, weighted by the confusion probability.
                # We want to minimize this value, effectively pushing them further apart.
                confusion_prob = cm_prob[minority_cls, majority_cls]
                if query_mask_source[minority_cls] > 0:
                    loss_cat += confusion_prob * torch.exp(logits_s[minority_cls, majority_cls])
                    num_pairs += 1
                if query_mask_target[minority_cls] > 0:
                    loss_cat += confusion_prob * torch.exp(logits_t[minority_cls, majority_cls])
                    num_pairs += 1

        loss_cat = loss_cat / (num_pairs + 1e-8)
        return loss_contrast + loss_cat * cat_weight
    ...
    # ----全局原型损失-----
    def loss_contrast_da(self, outputs, **kwargs):  # Original function is now replaced by the CAT version
        """
        This function is now a wrapper for the new CAT-inspired contrastive loss.
        """
        cat_weight = kwargs.get('cat_weight', 1.0)
        return self.loss_contrast_da_cat(outputs, temperature=0.2, cat_weight=cat_weight)
    ...
    def forward(self, outputs, targets, return_indices=False, target_domain_flag=False):
        ...
        if 'da_output' in outputs:
            ...
            kwargs = {}
            if hasattr(self, 'cat_weight'):
                kwargs['cat_weight'] = self.cat_weight
            losses['loss_global_proto_DA'] = self.loss_contrast_da(da_output['global_proto_DA'], **kwargs)
        # ======================================
        if return_indices:
...
def build_dino(args):
    ...
    model = DINO(
        ...
        cm_warmup_epochs=getattr(args, 'cm_warmup_epochs', 10),
    )
    if args.masks:
    ...
    criterion.cat_weight = getattr(args, 'cat_weight', 1.0)  # Pass cat_weight to criterion
    criterion.to(device)
