    ...
    for samples, targets, _, _ in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                ...
                outputs = model(samples, targets, epoch=epoch)
            else:
...
def train_one_epoch_with_self_training(model: torch.nn.Module, teacher_model: torch.nn.Module,
                                       ...
                                       data_loader: Iterable, data_loader_strong_aug: Iterable,
    ...
    for samples, source_labels, target_labels, samples_strong_aug in metric_logger.log_every(use_data_loader,
                                                                                             print_freq, header,
                                                                                             logger=logger):
        """
        ...
        orig_unlabel_target_sizes = torch.stack([torch.tensor([1, 1]).to(device) for i in range(len(target_labels))],
                                                dim=0)  # 保证坐标归一化
        target_predict_results = postprocessors['bbox'](target_predict_results, orig_unlabel_target_sizes,
                                                        not_to_xyxy=True)  # [{'scores': s, 'labels': l, 'boxes': b}]

        # -------------2.使用固定阈值得到伪标签-------------
        # 2.1 设置各类别置信度阈值
        threshold = np.asarray([args.pseudo_label_threshold] * (args.num_classes))  # 去除背景类
        # 2.2 卡阈值以得到可靠伪标签
        idx_list, labels_dict, boxes_dict, scores_dcit = get_pseudo_label_via_threshold(target_predict_results,
                                                                                        threshold=threshold)
        # 2.3 将伪标签处理为计算损失的格式
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                outputs = model(samples_strong_aug, source_labels, self_training_flag=True, epoch=epoch)
            else:
            ...
            valid_target_outputs, target_pseudo_labels = get_valid_output(target_outputs, target_pseudo_labels,
                                                                          idx_list)
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False,
    ...
    # By default, useCats is True. It will be False only if --useCats is explicitly set to False in config or options.
    ...
    for samples, _, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                ...
                outputs = model(samples, targets, epoch= 0) # 评估时epoch可以为0
            else:
                outputs = model(samples)
            # outputs = model(samples)
    ...
    if coco_evaluator is not None:
        ...
        # coco_evaluator.summarize() # This only prints the summary
        # The following loop will print per-category AP
        print("\nPrinting per-category AP:")
        # 从评估器中获取bbox的评估结果对象
        coco_eval = coco_evaluator.coco_eval['bbox']

        # 获取所有类别的ID
        # 创建一个从类别ID到类别名的映射
        cat_id_to_name = {cat['id']: cat['name'] for cat in base_ds.cats.values()}

        # `eval` 属性包含了所有详细的评估结果
        # precision 矩阵的形状是 (T, R, K, A, M)
        # T: 10个IoU阈值 (0.50-0.95)
        # R: 101个Recall阈值
        # K: 类别数量
        # A: 4个面积范围 (all, small, medium, large)
        # M: 3个maxDets (1, 10, 100)
        precision = coco_eval.eval['precision']

        # 我们关心的是 IoU=0.50, area=all, maxDets=100 的情况
        # precision 矩阵的第一个IoU阈值 (T=0) 对应 IoU=0.50
        # 我们对 Recall 维度求平均 (R)，并选择 A=0 (all), M=2 (100)
        # 结果是一个长度为 K (类别数) 的向量
        ap_per_category = precision[0, :, :, 0, 2].mean(axis=0)

        # coco_eval.eval['precision'] 的类别维度顺序与 coco_eval.params.catIds 一致
        # 我们需要根据这个顺序来匹配类别名称
        eval_cat_ids = coco_eval.params.catIds
        if len(ap_per_category) == len(eval_cat_ids):
            print("\nPrinting per-category AP:")
            for i, cat_id in enumerate(eval_cat_ids):
                cat_name = cat_id_to_name.get(cat_id, f'unknown_id_{cat_id}')
                ap = ap_per_category[i]
                print(f"  - AP@50 for class '{cat_name}': {ap:.4f}")
        else:
            print("\nWarning: Mismatch between number of categories in evaluation and AP results. Skipping per-category AP print.")
        print("\n")
